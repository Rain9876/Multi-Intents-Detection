...
class BertZSL(nn.Module):
    ...
    def __init__(self, config, num_labels=2):
        ...
        # self.bert = AutoModel.from_pretrained('sentence-transformers/bert-base-nli-stsb-mean-tokens', output_hidden_states=True, output_attentions=True)
    ...
    def forward(self, x_caps, x_masks, y_caps, y_masks, labels):
        ...
        """
        # label encoder:
        last_hidden, clusters, hidden, att = self.bertlabelencoder(y_caps, attention_mask=y_masks, return_dict=False)

        last_hidden_states, pooled_output, hidden_states, attentions = self.bert(x_caps, attention_mask=x_masks, return_dict=False)
